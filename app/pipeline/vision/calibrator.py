...
class BreadboardCalibrator:
    ...
    def _cluster_1d(values: np.ndarray, expected_count: int) -> np.ndarray:
        ...
        if len(sorted_vals) < expected_count:
            return sorted_vals
        samples = sorted_vals.astype(np.float32).reshape(-1, 1)
    ...
    def _build_from_peaks(
        ...
        estimated_rows = max(1, round((r_max - r_min) / row_pitch) + 1)
        target_rows = self._expected_rows
        self._row_coords = np.linspace(r_min, r_max, target_rows)

        # 计算空间哈希参数
        self._compute_grid_params()

        logger.info(
            "[Calibrator] estimated_rows=%d, using_expected_rows=%d, pitch=%.1f, range=[%.0f, %.0f]",
            estimated_rows,
            target_rows,
            row_pitch,
            r_min,
