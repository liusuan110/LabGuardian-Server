...
| **R9** | **Sim → real Phase 3**: real student netlist ingestion. Loader + evaluator + CLI + nightly hook + drift-table column, all gated on a single directory layout so the system "just works" when production data lands. | GNN / evaluator | P1 | ✅ shipped (2026-05-18) — `app/domain/gnn/real_netlist_loader.py` (permissive loader, LoadStats observability), `evaluator.evaluate_real_samples`, `scripts/gnn_eval.py --real-dir` flag, nightly auto-detects `REAL_DIR` (default `datasets/real_student/`), drift table adds "real" column when `checkpoints/p5_eval_real/metrics.json` exists. 19 new tests (547 total). Smoke validated against 5 hand-rolled fixtures in `tests/fixtures/real_student_simulated/` (rule accuracy 100%). See [`docs/REAL_STUDENT_NETLIST.md`](../../../docs/REAL_STUDENT_NETLIST.md) for the schema contract teachers / S3 should target. |
| **R10** | **Curriculum mismatch — opamp_inverting fixture**: `test_opamp_inverting_v1.json` only had R_in + R_f with pin 3 wired directly to GND. Standard textbook canon (Sedra/Smith, university lab manuals) includes a **third resistor R_p** between pin 3 and GND for input-bias-current compensation. Students following the textbook were flagged as FAIL with `CRITICAL_EXTRA_CONNECTION` because R_p touches GND. Surfaced when the user shared the lab manual image of an inverting amplifier — see chat log 2026-05-18. | Curriculum / GNN | P0 (silent false_fail for textbook students) | ✅ shipped (2026-05-18, Path A from RULE_SEMANTICS §6.4 R6 family) — fixture updated to include R_p + V_p net (4 components / 14 ports / 6 nets); 600 opamp_inverting samples regenerated; SEAL dataset re-prebaked (281 MB / 86518 rows); GNN retrained as `checkpoints/p3_followup_v3` (**val F1 0.959 ↑ from 0.950, test F1 0.994**); advisor default-ckpt priority bumped. **All gates still pass: 547 tests green, nightly 0.0000 false_pass on every split, sim→real drift all 18 cells clean**. 5-variant smoke validates: textbook 3-R → PASS; missing R_p → FAIL (GNN pin-points `U1.3 → n_gnd` at p=0.005); 3 miswire variants still caught with GNN p<0.17 on the broken edge. |
| **R11** | **Hole-move → netlist refresh** audit (2026-05-19). User reported that manual hole-corrections via the frontend sometimes don't form a new electrical connection. Three independent issues found across the backend pipeline: **B0** netlist_v2 only exposed `node_index` for holes-with-pins so frontend couldn't see the full conducting strip on drag; **B1** five sites used `pin.electrical_node_id or board_schema.resolve_hole_to_node(hole_id)` — stale upstream values shadowed the schema's recomputation; **B2** default rail labels lived as a module-level constant in `s3_topology.py` instead of on the BoardSchema. | Pipeline / Frontend contract | **P0** (silent connectivity bug for the manual-correction UX) | ✅ shipped (2026-05-19) — **B0**: `NetlistV2.board_topology` carries the full schema `node_to_holes` + effective `rail_assignments`; `BoardSchema.node_to_hole_index()` returns 136 nodes covering all 884 schema holes (e.g. `ROW_5_L: [A5,B5,C5,D5,E5]`). **B1**: all 5 sites inverted to `resolve(hole_id) if hole_id else stale` (`s2_mapping.py:122`, `topology_input.py:110`, `circuit.py:205/275/636`); also fixed `tests/pipeline/test_logical_reference_validate.py::test_open_circuit_detected` which had been pinning the bug (E3 mis-labelled as ROW_3_R; corrected to F3). **B2**: `BoardSchema.default_rail_assignments()` is the canonical source; `s3_topology.run_topology` now echoes `effective_rails` back through `board_topology.rail_assignments`. 7 new pin-the-contract tests in `tests/domain/test_board_topology_export.py`. **554 tests green** (547 → 554). All sim→real / nightly gates still 0.0000 false_pass. |
