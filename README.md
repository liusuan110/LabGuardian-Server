...
当前主线已经从早期 demo 形态，迁移到以下双轨并行架构：

- 旧链路：`pin1_logic / pin2_logic -> legacy netlist`
- 新链路：`component_id + pin_name + hole_id -> electrical_node_id -> electrical_net_id -> netlist_v2`
...
```text
S1 检测 -> S2 pin1_logic/pin2_logic -> S3 legacy analyzer -> legacy netlist
```
...
- 如果必须兼容旧链路，兼容逻辑统一收口在 `topology_input.py`
...
| `src_v2/logic/circuit.py` | `app/domain/circuit.py` | 已升级为双栈兼容 |
| `src_v2/logic/validator.py` | `app/domain/validator.py` | 已升级为 `validator_report_v2` |
