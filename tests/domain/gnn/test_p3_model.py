...
def test_train_full_cli_exits_3_when_any_gate_fails(tmp_path: Path) -> None:
    ...
    refs_config_path.write_text(json.dumps(cfg), encoding="utf-8")
    ...
    n = len(all_entries)
    (splits_dir / "train.json").write_text(json.dumps(all_entries[: n // 2 or 1]), encoding="utf-8")
    (splits_dir / "val.json").write_text(json.dumps(all_entries[n // 2 : n - 1] or all_entries[:1]), encoding="utf-8")
    (splits_dir / "test.json").write_text(json.dumps(all_entries[-1:] or all_entries[:1]), encoding="utf-8")
