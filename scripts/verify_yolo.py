...
验证路径:
  ...
  2. ComponentDetector 能加载权重不报错
  3. 对纯黑图调用 detect() 不崩溃
  4. model_contract.names 包含 ic-8 / ic-14
  5. normalize_component_type 映射正确
...
try:
    ...
    for ct in ("IC8", "IC14"):
        ...
        check(f"default_pin_schema_id({ct}) → {expected_schema!r}", schema == expected_schema, f"got {schema!r}")
